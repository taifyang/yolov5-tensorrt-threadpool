...
static __global__ void decode_kernel(float* predict, int num_bboxes, int num_classes, float score_threshold, float* inverse_affine_matrix, float* parray, int max_objects, int num_box_element)
{
...
static __global__ void nms_kernel(float* bboxes, int max_objects, float threshold, int num_box_element) 
{
	...
	// left, top, right, bottom, score, class, keepflag
...
void decode_kernel_invoker(float* predict, int num_bboxes, int num_classes, float score_threshold, float* inverse_affine_matrix, float* parray, 
    int max_objects, int num_box_element, cudaStream_t stream)
{
	auto grid = grid_dims(num_bboxes);
	auto block = block_dims(num_bboxes);
	decode_kernel << <grid, block, 0, stream >> > (predict, num_bboxes, num_classes, score_threshold, inverse_affine_matrix, parray, max_objects, num_box_element);
}
